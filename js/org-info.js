/* org-info.js script from orgmode.org/org-info.js
 * written by Sebastian Rose
 */
...
OrgNode.textNodeToIdx = function (b, c) {
    while (
        b.nodeType != 1 ||
        -1 == b.attributes.id.value.indexOf("outline-container-")
    ) {
        b = b.parentNode;
    }
...
var org_html_manager = {
    ...
    UNTAGX: /<[^>]+>/i,
    ORGTAGX:
        /^(.*)<span\s+class=[\'\"]tag[\'\"]>(<span[^>]+>[^<]+<\/span>)+<\/span>/i,
    TRIMMER: /^(\s*)([^\s].*)(\s*)$/,
    TOC: null,
    RUNS: 0,
    H: new Array(50),
    ...
    setup: function () {
        ...
        d.STARTUP_MESSAGE =
            d.STARTUP_MESSAGE && d.STARTUP_MESSAGE != "0" ? true : false;
        ...
        d.LINKS +=
            (d.LINK_UP && d.LINK_UP != document.URL
                ? '<a href="' + d.LINK_UP + '">Up</a> / '
                : "") +
    ...
    init: function () {
        ...
        if (null == m.B) {
            if (5 > m.RUNS) {
                m.LOAD_CHECK = window.setTimeout(
        ...
        m.PREA = document.getElementById("preamble");
        m.POST = document.getElementById("postamble");
        if (null == m.PREA) {
            m.PREA = m.B;
        }
        if (null == m.POST) {
        ...
        m.C.style.zIndex = "9";
        m.C.style.border = "1px solid #cccccc";
    ...
    initFromTOC: function () {
        ...
        k.T = document.getElementsByClassName("title")[0];
    ...
    build: function () {
        ...
        for (var d = 0; d < this.S.length; ++d) {
            ...
            c +=
                '</td><td style="text-align:center;vertical-align:bottom;border-style:none;width:56%;">';
            if (d > 0 && this.S[d].P.P) {
                c +=
                    '<a href="' +
                    this.S[d].P.L +
            ...
            c +=
                '</td><td style="text-align:right;vertical-align:bottom;border-style:none;width:22%">';
            c += d + 1 + "</td></tr></table>";
            ...
            if (0 < this.S[d].CH.length && this.LOCAL_TOC) {
                ...
                if ("above" == this.LOCAL_TOC) {
                    if (this.S[d].F) {
                        ...
                        this.S[d].D.insertBefore(
                            a,
                            this.S[d].D.getElementsByTagName(
                                "h" + this.S[d].DEPTH,
                            )[0].nextSibling,
                        );
                    }
    ...
    adjustSlide: function (f, g) {
        ...
        if (k) {
            document.onclick = function () {
                org_html_manager.sCk(
                    "org_html_manager.nextSection(org_html_manager.N.J + 1)",
                );
            };
        } else {
            document.onclick = function () {
                org_html_manager.sCk(
                    "org_html_manager.adjustSlide(org_html_manager.N.J, +1)",
                );
            };
        }
        if (e) {
            ...
            document.ondblclick = function () {
                org_html_manager.sCk(
                    "org_html_manager.adjustSlide(" + this.N.J + ", -1)",
                );
            };
        }
    },
    ...
    sCk: function (b, a) {
        ...
        if (null == this.CTO) {
            this.CTO = window.setTimeout(
                "org_html_manager.executeClick(" + b + ")",
                a,
            );
        } else {
            ...
            if (a == "s") {
                ...
                if (a == "S") {
                    ...
                    if (a == "r") {
                        ...
                        j.warn(
                            "Search backwards: text not found.",
                            false,
                            j.OCCUR,
                        );
                        j.OCCUR = g;
